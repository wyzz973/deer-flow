...
- 导出：Markdown（`[n](#ref-n)` 与参考文献）、HTML（markdown-it 渲染，引用上标，导出响应带严格 CSP）、
  Word（`docx_document`，见 9.2.1）。历史 AST 报告由 `render.docx_report` 导出。

#### 9.2.1 Word 导出

面向"这份报告要发给人看"，与 Markdown / HTML 三者的引用呈现保持一致：

- 正文引用是上标超链接，跳到文末对应条目的书签；条目是真正的超链接，后面跟站点名、来源声明的发布日期，
  以及只有检索摘录时的「检索摘录，未读取原文」。这几项此前只有 Markdown 和 HTML 有。
- 文首是可点击目录（二、三级标题），页脚有页码，标题样式带 `eastAsia` 中文字体回退，
  演示模式提示在文首而不是文末。页边距 1 英寸，正文栏宽 6.5 英寸。
- 表格是**三线表**：只有顶线、表头下线与底线，没有竖线也没有底纹。全框线加灰底在纸上很吵，
  而且 Word 断表时方框会让接缝成为整页最显眼的东西。表头加粗并跨页重复，
  每一行 `cantSplit`（否则一行会被切开，在续页表头下留一截孤零零的文字），
  列宽固定均分（Word 的 autofit 会把一列挤成竖排单字）。
- Mermaid 图渲染成图片并带「图 N」题注。Mermaid 需要浏览器才能绘制，而本产品要在没有渲染服务、
  没有外网的内网里可用，所以由**已经显示这些图的页面**渲染后随 `POST /{id}/report/docx` 回传
  （`frontend/src/core/deepresearch/diagrams.ts`）。图片按**图自身的源码**归档，服务端据此与文档里的块匹配，
  因此一张图只可能落在它自己的位置上。
- 客户端传来的字节是不可信输入：必须是真 PNG、单张不超过 4 MB、单次不超过 60 张 / 24 MB，
  并同时受正文栏宽与页面高度约束（纵向流程图按原尺寸会有十几英寸高，Word 会在分页处裁掉）。
- 一张竖版页只能容纳"高不超过宽约 1.4 倍"的图。按页高缩放后宽度不足正文栏 55%（`NARROW_DIAGRAM`）的图
  **单独起一页**：否则它会把自己挤到下一页，在前一页留下大半页空白，而占的纸是一样的。
  图与题注之间设 `keep_with_next`，题注不会被甩到下一页。
  根因在图本身的比例，所以章节提示词（`prompts.py`）要求优先用 `flowchart LR`、控制在八个节点左右。
- 没有可用图片的块保留题注，源码收进附录——`GET ...&format=docx` 也走这条降级路径，
  供没有浏览器的脚本与 CLI 使用。
- 浏览器绘制导出用的图时关闭 `htmlLabels`：Mermaid 默认用 `<foreignObject>` 排版标签，
  这样的 SVG 画进 canvas 会让浏览器判定画布被污染而拒绝导出，结果是一张图都出不来。
