...
- `kind: mcp` 的数据源直接把某个 MCP 工具原样暴露给研究员（保留它自己的参数 schema）。它的每次调用整体作为一条证据
  （标题是“数据源名: 查询词”）；想要“每条记录一条引用、带标题和链接”，用 `providers: [{type: mcp, ...}]` 的写法。
  两种写法的调用都计入 `max_searches_per_unit` 和 `max_tool_calls`。
