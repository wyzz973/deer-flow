...
输出三个文件，内容是同一份事实，默认写到 `.deerflow/deepresearch/audits/`（git 已忽略；审计会引用私有的研究内容，不要放进会被提交的目录）：

- `audit-<run8>.md`：底稿，给你读。
- `audit-<run8>.json`：给程序和 `--baseline` 对比用，也含每一次工具调用（`tool_log`）。
- `audit-<run8>.html`：给人看的单文件页面（样式、图表都内联，离线和内网都能打开）：时间线、每个阶段和节点的时间 / Token / 工具调用、每个研究步骤的时间构成与上下文增长、研究轮次与补研的成本和产出、供应商与搜索耗时分布、逐条工具调用（可筛选）。交付审计时把它和报告一起给用户。

数字来自产品自己的指标代码（`deepresearch.metrics.collect`），和页面“指标”页签一致；脚本另外补了产品没有的分析：Agent 循环的上下文增长、供应商逐轮缓存核对、前缀断点、最慢调用、各节点输出速度、补研与修复的触发原因、**失败的工具调用在时钟上的代价**（一轮里并行的工具要等最慢的一个，脚本按批次重算“去掉失败后”的等待，并推算每轮研究的时长）、**每个研究轮次的成本与产出**（时间、Token、工具调用，对比它第一次读到的证据里有多少被报告引用）。
...
5. **过程**：补研因为什么触发、哪些步骤被提前收尾、章节修复了几次、结束时还剩什么缺口；“研究轮次”表里补研轮花了多少、报告引用了它多少。
6. **交付物**：打开报告，对照研究请求逐项检查用户要的东西有没有交付（要的对比表有没有、覆盖全不全、空单元格多不多）。这一步脚本做不了，而它常常是最重要的发现：一次研究可以指标全绿、引用很多，却没有回答问题。缺了什么，就去看负责那部分的步骤最后一轮笔记（`show_call.py`），研究员通常会写明没拿到什么、为什么。
...
按 [references/audit-report.md](references/audit-report.md) 的模板写，默认中文。每个结论都带数字和它的出处（底稿的哪张表、哪个 call_id）；每条建议都落到具体设置和预期变化；没有验证过的推断明确标为推断。报告和底稿放在同一个输出目录，文件名 `审计报告-<run8>.md`；写完后**把第 2 步的命令原样再跑一次**，页面顶部就会带上你的结论（也可以用 `--report <文件>` 指定）。用户指定了文件名或位置就照用户的。你所在的环境不允许写文件时（有些子 Agent 环境如此），把报告全文作为回复返回，并说明底稿的位置。
